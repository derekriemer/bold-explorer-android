...
class WaypointRepositoryImpl @Inject constructor(
    private val db: BoldExplorerDatabase,
) : WaypointRepository {

    override fun observeAll(): Flow<List<Waypoint>> =
        db.waypointQueries.getAll().asFlow().mapToList(Dispatchers.IO).map { list -> list.map { it.toModel() } }
    ...
    override suspend fun getAll(): List<Waypoint> =
        db.waypointQueries.getAll().executeAsList().map { it.toModel() }

    override suspend fun getById(id: Long): Waypoint? =
        db.waypointQueries.getById(id).executeAsOneOrNull()?.toModel()
    ...
    override suspend fun withDistanceFrom(
        ...
        val candidates = if (bbox.needsLonFilter && bbox.crossing) {
            db.waypointQueries.waypointsInBboxCrossing(
                latMin = bbox.latMin,
                latMax = bbox.latMax,
                lonMin = bbox.lonMin,
                lonMax = bbox.lonMax,
            ...
            db.waypointQueries.waypointsInBboxNoCrossing(
                latMin = bbox.latMin,
                latMax = bbox.latMax,
                lonMin = lonMin,
                lonMax = lonMax,
        ...
        val trailWaypointIds: Set<Long>? = if (trailId != null) {
            db.trailWaypointQueries.getByTrail(trailId).executeAsList()
                .map { it.waypoint_id }.toSet()
        } else null
    ...
    override suspend fun attach(trailId: Long, waypointId: Long, position: Int?) {
        db.transaction {
            val pos = if (position == null) {
                db.trailWaypointQueries.nextPosition(trailId).executeAsOne().toInt()
            } else {
    ...
    override suspend fun detach(trailId: Long, waypointId: Long) {
        db.transaction {
            val pos = db.trailWaypointQueries.getPosition(trailId, waypointId)
    ...
    override suspend fun setPosition(trailId: Long, waypointId: Long, position: Int) {
        db.transaction {
            val a = db.trailWaypointQueries.getPosition(trailId, waypointId)
                .executeAsOneOrNull() ?: return@transaction
