...
class AutoWaypointRepositoryImpl @Inject constructor(
    private val db: BoldExplorerDatabase,
) : AutoWaypointRepository {

    override suspend fun forTrail(trailId: Long): List<AutoWaypoint> =
        db.autoWaypointQueries.forTrail(trailId).executeAsList().map { it.toModel() }
    ...
    override suspend fun create(
        ...
        db.transaction {
            db.autoWaypointQueries.insert(
                trailId, name, segmentIndex.toLong(), offsetM, lat, lon,
...
private fun com.boldexplorer.db.Auto_waypoint.toModel() = AutoWaypoint(
    id = id,
    trailId = trail_id,
    name = name,
